            ...
            try:
                ...
                try:
                    ...
                    with DB(host=db[0], port=db[1], user=db[2], passwd=db[3], db=db[-1]) as dbs:
                        ...
                        sql = "insert into {0}{1} values {2}".format(key,str(c_).replace("[", "(").replace("]",")").replace("'","")
...
class DriveForSettingDown(Task):
    ...
    def run(self, *arg, **kwarg):
        ...
        # print(down_topic)
        self.mqttclient = client(client_id=str(time.time()))
        self.mqttclient.mqttc.on_connect=self.on_connect
    ...
    def on_message(self, client, userdata, msg):
        ...
        if topic == "device/create":
            try:
                payload=json.loads(msg.payload.decode("gbk"))
                if payload.get("func_name") == "create_update_device":
