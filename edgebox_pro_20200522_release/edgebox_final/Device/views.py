...
from .models import SubDevice, EquipmentTemplateRtu, applyTemplateRtuInterface, applyTemplateTcpInterface, \
    ...
    EquipmentTemplateMqtt, applyTemplateMQTTInterface
        ...
        try:
            ...
            if request.GET.get("select") is not None:
                ...
                try:
                    ...
                    event=get_event_model.objects.create(event_level="info", event_remark=remark, event_username="admin")
                    event.save()
                    return JsonResponse({
                        "status_code": 1,
                        "message": "修改成功！"
                    ...
                    event=get_event_model.objects.create(event_level="error", event_remark=remark, event_username="admin")
                    event.save()
                    return JsonResponse({
                        'status_code': 1,
                        "error": "修改失败！"+str(e)
    ...
    if request.method == "POST":
        ...
        if request.GET.get("more") is not None:
            ...
            for subdevice_name in subdevice_name_list:
                ...
                cache =set_disenable(subdevice=subdevice_name) if enable else conn.hset(subdevice_name, "device_enable", 1)
...
def List(request):
    ...
    def status(row):
        ...
        res= 0 if conn.hget(row["subdevice_name"], 'drive_enable') is None else int(conn.hget(row["subdevice_name"], 'drive_enable'))
        if res:
            ...
            if vue_json.get("type") == "modbus":
                ...
                for row in vue_json.get("code_list"):
                    ...
                    etc = EquipmentTemplateCanis.objects.create(etc_name=template_name,
                                                              ...
                                                              etc_round=row["round"])
                    etc.save()
            elif vue_json.get("type") == "MQTT":
                for row in vue_json.get("code_list"):
                    # print(row)
                    etc = EquipmentTemplateMqtt.objects.create(etm_name=template_name,
                                                              etm_remark=template_remark,
                                                              etm_accordname=accordname,
                                                              etm_param=row["property"],
                                                              etm_param_as=row["as_name"],
                                                              etm_type=row["type"],
                                                              etm_null=row["is_null"])
                    etc.save()
...
def templateList(request):
    ...
    def select(rows, dtype):
        ...
        if dtype == "modbus":
            ...
            for etm in rows:
                template = EquipmentTemplateMqtt.objects.filter(etm_name=etm["etm_name"])
                # print(template)
                data = {}
                data["name"] = etm["etm_name"]
                data["protocol"] = etm["etm_accordname"]
                data["description"] = etm["etm_remark"]
                data["info"] = []
                # data["info2"] = []
                row = 0
                for index, tmp in enumerate(template):
                    ...
                    if index % 4 == 0:
                        data["info"].append({})
                        row += 1
                    data["info"][row - 1][index] = {"property": tmp.etm_param,
                                                    "as_name": tmp.etm_param_as,
                                                    "type": tmp.etm_type,
                                                    "is_null": tmp.etm_null
                                                    }
    ...
    if request.method == "GET":
        ...
        if subdevice != None:
            ...
            if template == "rtu":
                ...
                apply_mqtt = applyTemplateMQTTInterface.objects.filter(apply_mqtt_device=subdevice)
                etms = EquipmentTemplateMqtt.objects.values("etm_name", "etm_accordname", "etm_remark", ) \
                    .filter(etm_name=apply_mqtt[0].apply_mqtt_template).annotate(num=Count("etm_name"))
                templateList = select(etms, "MQTT")
                return JsonResponse({
                    "status_code": 0,
                    "db": templateList,
                    "drive_status": drive_status
        ...
        if vue_List == []:
            ...
            etms = EquipmentTemplateMqtt.objects.values("etm_name", "etm_accordname", "etm_remark", ) \
                .all().annotate(num=Count("etm_name"))
            templateList3 = select(etms, "MQTT")

            templateList = templateList1 + templateList2 + templateList3
        ...
        elif vue_List[1] == "MQTT":
            etms = EquipmentTemplateMqtt.objects.values("etm_name", "etm_accordname", "etm_remark", ) \
                .filter(etm_accordname=vue_List[1]) \
                .annotate(num=Count("etm_name"))
            # print(etrs)
            templateList = select(etms, "MQTT")
            return JsonResponse({
                "status_code": 1,
...
# apis/device/mqttForm
@csrf_exempt
def mqttForm(request):
    '''
        用于提供mqtt的from表单select
        :param request:
        :return:
        '''
    if request.method == "GET":
        form = {}
        drive_name = Drive.objects.filter(drive_type="MQTT").values("drive_name")
        driveList = [{"index": drive["drive_name"], "value": drive["drive_name"]} for drive in drive_name]
        form["driveList"] = driveList
        return JsonResponse({
            "status_code": 0,
...
def canisproForm(request):
    ...
    if request.method == "GET":
        ...
        bufferSizeList = [ {"index": str(v), "value": "HPS_BUFFER_SIZE"} for v in ("128K", "256K", "512K", "1M", "2M", "4M", "8M", "16M")]
        sampleRatelist = [ {"index": str(v)+"K", "value": "HPS_SAMPLE_RATE"} for v in (1, 2, 4, 8, 16, 32, 64, 128, 256)]
...
def dbData(request):
    ...
    if request.method == "GET":
        try:
            subdevice_name = request.GET.get("subdevice_name")
            templatename = request.GET.get("templatename")
            param = request.GET.get("param")
            
            conn = get_redis_connection('default')
            cls = get_subdevicedata_model(subdevice_name)
            
            # 最近的n=1000筆數據
            row_last_data = cls.objects.values("data", "id", 'create_time').order_by("-id")[:1000]
            
            db = [json.loads(i["data"].replace("'", '"')) for i in row_last_data]
            varlist = []
            try:
                varlist = eval(conn.hget(subdevice_name, templatename + '_varlist').decode())
                if param is None:
                    param = varlist[0]
            except:
                conn.hset(subdevice_name, templatename + '_varlist', [])
            ...
            var_data = [i[param] for i in data_db]
            var_data.reverse()
            
            content = {
                "status_code": 0,
                'value': var_data,  #
                'data_index': np.arange(1000).tolist(),
                'params': varlist
            }
...
def fileData(request):
    ...
    if request.method == "GET":
        try:
            ...
            hex_path = os.path.join(os.path.abspath(r"plugins\CanisPro\HEX"), file+".txt")
            data_ac=etl_hex2dc_csv(hex_path)[:10000]
            return JsonResponse({
                "status_code": 0,
                "data_ac": data_ac.tolist(),
                "data_index": np.arange(data_ac.size).tolist()
            })
            # data=pd.read_pickle("testtest.pk")
            # L=data["Sensor1"].values.tolist()[69500:71500]
            # return JsonResponse({
            #     "status_code": 0,
            #     "data_ac": L[:],
            #     "data_index": np.arange(len(L)).tolist()
            # })
...
def Data(request):
    ...
    if request.method == "GET":
        ...
        if row:
            ...
            try:
                print("row:", row)
                if conn.hget(subdevice, "template").decode() == "canis":
                    canis_files = [f for f in os.listdir("plugins/CanisPro/HEX") if "data_ch" in f]
                    # sort timestamp
                    canis_files.sort(key=lambda x: -int(x[9:-4]))
                    # "KB View as Dialog"
                    hexpath = os.path.abspath("plugins/CanisPro/HEX")
                    tablaData = [{"property": f[:-4],
                                  "type":"View as Dialog",
                                  "value": str(os.path.getsize(os.path.join(hexpath, f))//1024)+"KB",
                                  "create_time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(f[9:-4])))}
                                 for f in canis_files]
                    Total = len(tablaData)
                    return JsonResponse({
                        "status_code": 0,
                        "tableData": tablaData,
...
def showApply(request):
    ...
    def canis(apply_canis, res):
        ...
        return apply_interface
    def mqtt(apply_mqtt, res):
        active =False if res == 0 else True

        apply_interface=[]
        apply_interface.append(["驱动名称", apply_mqtt[0].apply_mqtt_drive])
        apply_interface.append(["驱动类型", "MQTT"])
        apply_interface.append(["驱动状态", active])
        apply_interface.append(["IP地址", apply_mqtt[0].apply_mqtt_ip])
        apply_interface.append(["端口号", apply_mqtt[0].apply_mqtt_port])
        apply_interface.append(["用戶名", apply_mqtt[0].apply_mqtt_userName])
        apply_interface.append(["訂閱主題", apply_mqtt[0].apply_mqtt_topic])
        return apply_interface
    ...
    if request.method == "GET":
        ...
        template = None if conn.hget(subdevice, "template") is None else conn.hget(subdevice, "template").decode()
        drive_status_msg = '' if conn.hget(subdevice, "drive_status_msg") is None else conn.hget(subdevice, "drive_status_msg").decode()
        if template == "rtu":
            apply_rtu = applyTemplateRtuInterface.objects.filter(apply_rtu_device=subdevice)
            ...
            return JsonResponse({
                "status_code": 1,
                "db": mqtt(apply_mqtt, res),
                "template_name": apply_mqtt[0].apply_mqtt_template,
                "drive_status_msg": drive_status_msg
...
def templateApply(request):
    ...
    if request.method == "POST":
        ...
        if vue_json.get("type") == "rtu":
            ...
            applyTemplateRtuInterface.objects.update_or_create(apply_rtu_device= vue_json["subdevice_name"], defaults=update_content)
            ...
            update_content = {
                "apply_mqtt_template": vue_json["template_name"],
                "apply_mqtt_ip": vue_json.get("content")["ip"],
                "apply_mqtt_port": vue_json.get("content")["port"],
                "apply_mqtt_userName": vue_json.get("content")["userName"],
                "apply_mqtt_pwd": vue_json.get("content")["pwd"],
                "apply_mqtt_topic": vue_json.get("content")["topic"],
                "apply_mqtt_drive": vue_json.get("content")["drive"],
                "apply_mqtt_active": False
            }
            applyTemplateMQTTInterface.objects.update_or_create(apply_mqtt_device=vue_json["subdevice_name"],
                                                               defaults=update_content)
            conn.hset(vue_json["subdevice_name"], "template", "mqtt")
            conn.hset(vue_json["subdevice_name"], "template_name", vue_json["template_name"])
