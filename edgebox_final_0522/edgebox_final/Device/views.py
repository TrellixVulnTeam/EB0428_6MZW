...
def pathList(request):
    ...
    if request.method == "POST":
        ...
        if request.GET.get("select") is not None:
            ...
            elif "Result Down" in vue_json["path_type"]:
                vue_json = vue_json["params"]
                print("Result Down")
                path = Path.objects.create(path_name=vue_json["name"], path_type="Result Down",
                                           subdevice_name=subdevice, path_ip=vue_json["ip"],
                                           path_port=vue_json["port"], path_sub=vue_json["pubtopic"], path_enable=True)
                path.save()
                conn.hset(key, "path_N_5", 0)
                conn.hset(key, "spd_count", 0)
                conn.hset(key, "path_bpd_count", 0)
                conn.hset(key, "path_type", "Result Down")
                conn.hset(key, "path_name", vue_json["name"])
                conn.hset(key, "path_host", vue_json["ip"])
                conn.hset(key, "path_port", vue_json["port"])
                conn.hset(key, "path_username", vue_json["userName"])
        ...
        if row:
            ...
            try:
                ...
                if conn.hget(subdevice, "template").decode() == "canis":
                    ...
                    if varlist == []:
                        tablaData = [{"property": i, "value": v, "create_time": row_last_data["create_time"]} for i, v
                                     in data.items()]

                    else:
                        tablaData = [{"property": i, "value": v, "create_time": row_last_data["create_time"]} for i, v
                                     in data.items() if i in varlist]
                    Total = cls.objects.count()
