...
from Device.models import applyTemplateRtuInterface, applyTemplateTcpInterface, EquipmentTemplateRtu, \
    applyTemplateCanisInterface, EquipmentTemplateCanis
from Drive.models import Drive
from Drive.tasks import DriveModbusRtu, DriveModbusTcp, DriveCanisPro
    ...
    def apply_interface():
        ...
        if vue_json["drive_type"] == "Modbus-RTU":
            ...
            interface = applyTemplateCanisInterface.objects.filter(apply_canis_drive=vue_json["drive_name"],
                                                                   apply_canis_device=vue_json["device_name"]).values()[
                0]
            return interface
    ...
    if request.method == "POST":
        ...
        if vue_json["drive_type"] == "Modbus-RTU":
            ...
            if vue_json["enable"]:
                ...
                result = DriveModbusRtu.apply_async(
                    kwargs={"apply_template": apply_template(), "apply_interface": apply_interface()},
                    queue="worker_queue")
                status = "设备采集驱动启动成功！"
                # 新增一条log
            ...
            if vue_json["enable"] or vue_json["enable"] in ["1", 1]:
                conn.hset(vue_json["device_name"], "drive_enable", 1)
