<template>
  <el-main class="no-padding">
        ...
        <el-table-column sortable label="创建时间" prop="createTime"></el-table-column>
         <el-table-column label="协议名称" prop="protocol">
          <template slot-scope="scope">
            ...
            <el-tag v-else-if="scope.row.protocol === 'MQTT'" size="medium" type="warning">{{ scope.row.protocol }}</el-tag>
     ...
     <div class="dialogfrommodel">
      <el-dialog   :title="'新增设备模板('+form1.typeSelected[1]+')'" :visible.sync="dialogFormCreateMQTT"  width= "680px">
              ...
              <template slot-scope="scope">
                    ...
                    :rules="[
                      ...
                      <el-select size="small" v-model="scope.row.is_null" placeholder="是">
                        <el-option v-for="item in appforms.NULL_type" :key="item.index" :label="item.value" :value="item.index">
                        </el-option>
                      </el-select>
                    </el-form-item>
                </template>          
        ...
        <el-row  slot="footer">
          ...
          <el-button type="primary" @click="templateForm.templatetype='MQTT'; submitForm('mqttform')" size="small">保存</el-button>
        </el-row>
      </el-dialog>
    </div>
    <!-- 应用模板弹出框 -->
    ...
    <el-dialog :title="'协议接口配置('+select_template+')'"
        :visible.sync="dialogFormMqtt" width="680px" custom-class="chile-device-box dialog-form"
    >
      <mqtt-form  :templatename ="select_template"
        @CloseDialog="closeDialogMqtt" >
      </mqtt-form> 
...
import mqttForm from '../components/MqttForm.vue';
       ...
       this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          case 'canispro':
              ...
              break;
          case 'MQTT':
            _this.dialogFormCreateMQTT= false;
            _this.dialogFormCreatedesc = false;
            _this.$axios.post('apis/device/template/create/', {
              code_list: this.mqttform.domains,
              template_name: this.templateForm.template_name,
              template_remark: this.templateForm.remark,
              type: "MQTT",
              accordname: this.form1.typeSelected
