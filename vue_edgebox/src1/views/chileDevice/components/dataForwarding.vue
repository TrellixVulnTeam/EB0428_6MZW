<template>
  <el-main class="no-padding">
    ...
    <el-row class="row-padding row" >
      <el-button type="primary" size="small" @click="dialogFormVisible = true">新增转发</el-button>
      <el-button type="primary" size="small" @click="dialogFormVisibleFtp = true">FTP文件传输</el-button>
    ...
    <el-table :data="tableData">
      ...
      <el-table-column min-width="10%" label="上行次数" prop="mesNumber"></el-table-column>
    ...
    <!--新增FTP路径传输 -->
       ...
       <el-dialog title="FTP路径传输设置" :visible.sync="dialogFormVisibleFtp" custom-class="chile-device-box dialog-form" width="560px">
      <el-form :model="ftpform" label-position="left" label-width="120px" :rules="ftprules" ref="ftpform">
        <el-form-item label="路径名称:" prop="name">
          <el-input type="text" v-model="ftpform.name" size="small" oninput="value=value.replace(/[^0-9^A-Z^a-z]/g, '')" placeholder="PATH NAME" @blur="inputName"></el-input>
        </el-form-item>
        <el-form-item label="FTP服务器:" prop="host">
          <el-input placeholder="IP" size="small" v-model="ftpform.host"></el-input>
          <el-select size="small" v-model="ftpform.port" placeholder="PORT" >
            <el-option label="21" value="21"></el-option>
          </el-select>
          <el-input type="text" v-model="ftpform.username" size="small"  placeholder="USERNAME"></el-input>
          <el-input type="password" v-model="ftpform.password" size="small"  placeholder="PASSWORD"></el-input>

        </el-form-item>
        <el-form-item label="选择文件夹:" prop="currentpath">
          <el-select size="small" v-model="ftpform.currentpath" placeholder="CURRENT PATH" >
            <el-option label="data" value="data"></el-option>
          </el-select>
          <el-input type="text" v-model="ftpform.orderpath" size="small"  placeholder="ORDER PATH"></el-input>
        </el-form-item>

      </el-form>
      <el-row  slot="footer">
        <el-button type="success" size="small" >测试</el-button>
        <!-- <el-button type="success" size="small" @click="test('ftpform')" >测试</el-button> -->
        <el-button type="primary" size="small" @click="reset('ftpform')" plain>重置</el-button>
        <el-button type="primary" size="small" @click="saveFtp('ftpform')">保存</el-button>
      </el-row>
    </el-dialog>
...
export default {
  data() {
    ...
    return {
      ...
      dialogFormVisibleFtp: false, // 新增Ftp弹出框
      ...
      ftprules: { // ftp表单验证
        ...
        host: [{required: true, validator: checkNull, trigger: 'blur'}],
        ...
        currentpath: [{required: true, validator: checkNull, trigger: 'blur'}],
        orderpath: [{required: true, validator: checkNull, trigger: 'blur'}],
      },
      rules: { // 新增转发表单验证
        name: [{required: true, validator: checkNull, trigger: 'blur'},
              { validator: dulapathname, trigger: 'blur'}],
