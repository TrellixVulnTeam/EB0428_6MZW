...
export default {
    ...
    methods:{
      getUserList(){
        var _this = this
        this.$axios.get("/user/user_info/" + JSON.parse(this.$secre.changeSecret(localStorage.getItem("tokenLogin")))["name"])
          .then(response => {
      ...
      deleteRow (row) {
        ...
        this.$Message.WarningAlert(this, '您确定要删除此用户吗？', '用户删除').then(
            res => {
              this.$axios.post("/user/delete_user",{
      ...
      save(row){
        ...
        if(row.exis == true){
          ...
          this.$axios.post("/user/create_user",{
            username: JSON.parse(this.$secre.changeSecret(localStorage.getItem("tokenLogin")))["name"],
