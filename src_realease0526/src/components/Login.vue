...
export default {
        ...
        $(document).off().click(function(e){})
            $(document).click(function(e){
                ...
                if(idValue == "el-login-switch"){  
                    ...
                    // });
                    _this.$axios.get('user/login/change')
                        .then(response => {
    ...
    methods:{
        ...
        refresh_verify_code(){
            this.image_url = "/user/verify_code/0." + Math.floor(Math.random() * 1000000000)
            // this.$axios.get('user/verifyApi')
            // this.$axios.get('/user/verify_code')
            //     .then(response=>{
            //     // this.image_url = "http://127.0.0.1:8000" + response.data.url
        ...
        activate_user(){
            var _this = this
            this.$axios.get('user/send_activate_email/' + this.form.uName).then(
                response=>{
                    if(response.data.error_code == 0){
