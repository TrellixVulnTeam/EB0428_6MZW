    ...
    <el-dialog title="应用" :visible.sync="dialogFormVisible" custom-class="chile-device-box dialog-form">
      <el-form :model="form" label-position="left" label-width="100px" :rules="rules" ref="form">
        ...
        <el-form-item label="回复超时:" prop="timeout">
          <el-input type="text" size="small" v-model="form.timeout">
            <el--button slot="append" type="text">S</el--button>
          </el-input>
        </el-form-item>
        <el-form-item label="读写周期:" prop="cycle" >
          <el-input type="text" size="small" v-model="form.cycle">
            <el--button style="margin-right: 10px" slot="suffix" type="text">S</el--button>
          </el-input>
        ...
        <el-form-item label="绑定驱动:" prop="drive">
          <el-select size="small" style="width: 100%" v-model="form.drive" placeholder="请选择所属类别">
            <el-option v-for="item in driveList" :key="item.index" :label="item.value" :value="item.index">
